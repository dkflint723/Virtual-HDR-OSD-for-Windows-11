...
5. Select the correct monitor under **1 · Target Display**.
6. Click **Import…** and load the Windows HDR Calibration `.icm` profile.
...
The top bar is numbered, and grouped by consequence — everything in row 2 stays
inside the application, everything in row 3 writes to your Windows configuration:

- **1 · Target Display** — selects and monitors the physical display, and opens the relevant Windows pages. Read-only, apart from **Compare SDR / HDR**.
- **2 · Base Profile & Edits** — imports the base profile, exports a copy, and manages your slider settings. **Nothing here changes your Windows configuration.**
- **3 · Apply to Windows** — installs and associates the generated HDR profile. These are the actions that take effect on your display.
...
> re-read the file shown in the profile path field, so clicking it can never
